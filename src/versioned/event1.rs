...
mod test {
    ...
    fn test_realworld_event_with_naddr_tag() {
        ...
        let _: EventV1 = serde_json::from_str(&raw).unwrap();
    }
    ...
    fn test_speedy_encoded_direct_field_access() {
        ...
        let pubkey = EventV1::get_pubkey_from_speedy_bytes(&bytes).unwrap();
        assert_eq!(pubkey, event.pubkey);

        let created_at = EventV1::get_created_at_from_speedy_bytes(&bytes).unwrap();
        assert_eq!(created_at, Unixtime(1680000012));

        let kind = EventV1::get_kind_from_speedy_bytes(&bytes).unwrap();
        assert_eq!(kind, event.kind);

        let ots = EventV1::get_ots_from_speedy_bytes(&bytes);
        assert_eq!(ots, None);

        let content = EventV1::get_content_from_speedy_bytes(&bytes);
        assert_eq!(content, Some(&*event.content));

        let re = regex::Regex::new("foodstr").unwrap();
        let found_foodstr = EventV1::tag_search_in_speedy_bytes(&bytes, &re).unwrap();
